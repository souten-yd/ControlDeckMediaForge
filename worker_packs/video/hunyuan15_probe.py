...
"""Local-only, bounded HunyuanVideo 1.5 adoption probe.
...
PRESETS = {
    # Loading/ROCm smoke only. It is never quality evidence.
    "smoke": ProbePreset(width=256, height=256, frames=5, steps=1),
    # Reduced short clip for host-memory and prompt-coherence comparison.
    "candidate-clip": ProbePreset(width=640, height=384, frames=33, steps=50),
    # Official-quality frame/step count for the 480p CFG-distilled model.
    "official-clip": ProbePreset(width=848, height=480, frames=121, steps=50),
}
...
def _snapshot(path: Path) -> Path:
    ...
    if not snapshot.is_dir() or snapshot.name != CANDIDATE_REVISION:
        raise ValueError("Hunyuan snapshot revision differs from the pinned revision")
    if snapshot.parent.name != "snapshots":
        raise ValueError("Hunyuan snapshot must use a verified Hugging Face cache layout")
    ...
    if not model_index.is_file() or not model_index.is_relative_to(repository):
        raise ValueError("Hunyuan model index escapes its repository")
    if model_index.stat().st_size > 64 * 1024:
        raise ValueError("Hunyuan model index is unbounded")
    try:
        value = json.loads(model_index.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("Hunyuan model index is unreadable") from exc
    if value.get("_class_name") != "HunyuanVideo15Pipeline":
        raise ValueError("Hunyuan snapshot has an unexpected pipeline class")
    return snapshot
...
def _save_frame(frame: Any, path: Path) -> None:
    ...
    if isinstance(frame, Image.Image):
        ...
        array = frame
        if getattr(array, "dtype", None) is None or getattr(array, "ndim", None) != 3:
            raise RuntimeError("Hunyuan returned an unreadable frame")
        if str(array.dtype).startswith("float"):
            array = (array.clip(0, 1) * 255).round().astype("uint8")
        elif str(array.dtype) != "uint8":
...
def _encode_frames(frames: Any, output: Path, preset: ProbePreset) -> None:
    if len(frames) != preset.frames:
        raise RuntimeError("Hunyuan returned an unexpected frame count")
    output.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="hunyuan15-frames-", dir=output.parent) as temporary:
        frame_root = Path(temporary)
        for index, frame in enumerate(frames):
            path = frame_root / f"{index:06d}.png"
            _save_frame(frame, path)
            from PIL import Image

            with Image.open(path) as image:
                if image.size != (preset.width, preset.height):
                    raise RuntimeError("Hunyuan returned an unexpected frame size")
    ...
    if completed.returncode != 0 or not output.is_file() or output.stat().st_size <= 0:
        ...
        raise RuntimeError("ffmpeg rejected the Hunyuan probe frames")
...
def _generate(snapshot: Path, output: Path, preset_name: str) -> dict[str, Any]:
    ...
    if not torch.cuda.is_available() or torch.version.hip is None:
        raise RuntimeError("HunyuanVideo 1.5 probe requires ROCm")
    properties = torch.cuda.get_device_properties(0)
    architecture = str(getattr(properties, "gcnArchName", ""))
    if architecture != "gfx1201":
        raise RuntimeError("HunyuanVideo 1.5 probe requires the target gfx1201 GPU")

    preset = PRESETS[preset_name]
    started = time.monotonic()
    pipeline = _load_pipeline(snapshot, torch, HunyuanVideo15Pipeline)
