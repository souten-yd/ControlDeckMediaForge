...
def test_workspace_websocket_chunk_import_exceeds_single_message_bound_and_cleans_up(tmp_path: Path):
    ...
    with client:
        ...
        with client.websocket_connect("/ws", headers=headers) as socket:
            ...
            assert list(work_dir.iterdir()), "the incomplete upload must exist before close"
            # TestClient context exit also cancels the ASGI task. Observe the
            # graceful disconnect cleanup BEFORE that forced cancellation.
            # Only poll filesystem state; never retry the failed import.
            socket.close()
            deadline = time.monotonic() + 5
