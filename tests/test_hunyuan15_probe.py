...
from worker_packs.video.hunyuan15_probe import (
    CANDIDATE_REVISION,
    NEGATIVE_PROMPT,
    PRESETS,
    PROMPT,
...
def snapshot(tmp_path: Path, *, class_name: str = "HunyuanVideo15Pipeline") -> Path:
...
def test_snapshot_requires_exact_revision_pipeline_and_contained_model_index(tmp_path: Path) -> None:
    ...
    outside.write_text(json.dumps({"_class_name": "HunyuanVideo15Pipeline"}), encoding="utf-8")
...
def test_pipeline_load_is_local_only_bfloat16_and_offloaded() -> None:
...
def test_frame_encoder_produces_bounded_h264_and_removes_temporary_frames(tmp_path: Path) -> None:
    ...
    frames = [Image.new("RGB", (preset.width, preset.height), (index * 20, 64, 128)) for index in range(5)]
    ...
    assert not list(tmp_path.glob("hunyuan15-frames-*"))
