...
def test_one_feedback_repair_includes_specific_edges_without_changing_constraints():
    ...
    gateway = Gateway(json.dumps(opened), json.dumps(draft_data()))
    result = asyncio.run(MeshDraftPreparer(gateway).prepare(IDENTITY, MeshDraftRequest(intent="armor")))
    assert [item["valid"] for item in result.provenance["attempts"]] == [False, True]
    feedback = json.loads(gateway.calls[1][2][-1]["content"])
    assert len(feedback["edge_problems"]) == 3
    assert all(item["use_count"] == 1 for item in feedback["edge_problems"])
    assert all("input" not in item and "ctx" not in item for item in feedback["issues"])
    assert gateway.calls[0][3]["response_format"] == gateway.calls[1][3]["response_format"]
