...
def observation_workspace(tmp_path: Path, *, mode: str = 'success') -> tuple[Store, SceneWorkspace, FakeResolver, SceneObserveRequest]:
    ...
    resolver.runtime.executable.write_text(
        ...
        f" pathlib.Path(view+'.png').write_bytes(base64.b64decode({base64.b64encode(buffer.getvalue()).decode()!r}))\n"
        " images.append({'view':view,'filename':view+'.png'})\n"
        "if mode=='bad_image': pathlib.Path(spec['views'][-1]+'.png').write_bytes(b'bad')\n"
        "if mode=='symlink':\n pathlib.Path('front.png').unlink()\n pathlib.Path('front.png').symlink_to('back.png')\n"
        ...
        "'observation':spec,'blender_version':'4.5.9','device':'CPU','frame':0,'samples':16,"
