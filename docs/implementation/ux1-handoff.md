...
正式Library/元画像/重み/managed rollback版は保持。詳細は[方向選択](multiview-direction-choice-20260923.md)。
...
詳細と再開コマンド: [0.33.7実機記録](release-0.33.7-20260922.md)。
...
詳細・scene/Asset ID・証跡: [0.33.6最終受入](release-0.33.6-20260922.md)。
...
最終 ./mf.sh test: 2410 passed / 3 warnings / 272.53秒。署名公開/通常更新/installed受入を継続。詳細: [実装と証跡](texture-scene-navigation-20260922.md)。
...
詳細・Asset ID・証跡・NOT TESTED: [0.33.2実機受入](release-0.33.2-20260922.md)。
...
詳細: [3D材質画像の派生候補](3d-texture-variants-20260922.md)。
...
詳細・NOT TESTED: [0.32.2 release受入](release-0.32.2-20260922.md)。
...
```text
PASS       ObservationSpec に clip_id / frames。クリップを当てて各フレームを描く
PASS       実機で walk の4フレームが全部別の絵。f0とf12は0%差、f6とf18は21%差
PASS       絵で脚の振れが見える。0.29.1の「f12は基本姿勢」の説明が裏付けられた
PASS       clip_idとframesは両方揃って初めて受ける。views×frames≤8
FOUND      worker側のvalidate_specにも足す必要があった（fail-closedが正しく効いた）
TODO       書く→描く→見る→直すを回す相手（ローカルLLM）はまだ動いていない
```
...
2. ローカルLLMを繋いで輪を実際に回す
3. 残: 6脚以外の形、Webの骨入れ操作、mesh_geometry facts の上限と法線焼き込み
```
...
`.blend` と `.glb` の 2 資産を登録するので、直すたびに一覧の行が 2 つ増えていた。

```text
PASS       実機 454 行が 85 枚に（全体 1421 → 1052）
PASS       束ねる鍵は scene_id。名前で束ねると同名の別シーン5件が混ざった
PASS       serverは事実だけ載せ、畳むのは画面（版は頁をまたいで届く）
PASS       代表はいちばん新しい版のGLB。名前はシーン名
PASS       選ぶあいだは束ねない。切り替えも無効
PASS       束ねたカードから開くと送り先はその版だけ
NOT TESTED 実機での確認（0.29.0を入れてから 3ds_library_revision_grouping_e2e.mjs）
```

## 2026-09-21 ライブラリの GLB サムネイル（0.28.99）

仕組みは既にあり、使われていなかった。実機は GLB 240 件のうち 12 件しか絵が無く、
その 12 件すら一覧では札になっていた（`libraryCard` が撮影済みの版を無条件に捨てていた）。

```text
PASS       撮れている版があれば一覧はそれを出す。札は撮れていないものだけ
PASS       絵の無いGLBが残っているときだけ行が出て、件数を言う
PASS       押すまで3Dランタイムもバイト列も運ばない（既存E2Eの設計判断を守る）
PASS       16MiB超は撮らない／1回24件／失敗を覚える／世代で降りる
PASS       撮影ごとにcanvasを作り直す（disposeはforceContextLossまで行く）
PASS       バイト列取得はstreamModelBytesの1か所だけ。契約テストで固定
NOT TESTED 実機での撮影（0.28.99を入れてから 3ds_library_glb_thumbnail_e2e.mjs を回す）
```
...
シーンを持たないライブラリの GLB（実機で 8 件）も同じ欄から選べるようにし、
選んだ時点で `scenes.from_glb` が編集できるシーンにする。

```text
PASS       選択欄がBlender panel内・ボタン直上、当たり判定44px（実Chrome 320px/1280px）
PASS       携帯で取り込みフォームがorder 1（編集入口の後ろ）
PASS       ライブラリGLBを選ぶとscenes.from_glbが飛び、シーン側へ移る
PASS       source_glb依存と来歴の親を記録。元GLBは使用中で削除不可
PASS       版のプレビュー済みGLBは二重作成を拒否
PASS       installed 0.28.94の配信HTMLに新UI、実DBで編集可能GLB 8件を確認
NOT TESTED 実iPhoneでの操作、実Blenderセッションでの編集
```

## 次にやること（1つだけ）

```text
1. 利用者に実iPhoneで新しい選択欄を触ってもらい、選ぶ→開くが一息でできるか確認する
2. ブラウザ拡張が繋がったら、実Blenderセッションで「指の操作」を実測する
```
...
詳細: [CPU remesh上限実測](g9-pixal-surface-budget-20260920.md)。全体目標は未完了。
...
詳細: [NAF Vulkan実測](g9-pixal-vulkan-naf-20260920.md)。
...
Pixal失敗は別途診断。詳細: [実GPU評価](g9-real-generation-20260920.md)。
...
詳細・証跡: [operator手順](g9-operator-session-20260920.md)、
...
詳細: [CPU確保実測](g9-flow-capacity-20260920.md)。
...
詳細は[operator手順](g9-operator-session-20260920.md)。今回の変更はbundle外のprivate評価補助であり、稼働0.28.88の変更なし。
...
次: [G9統合と残る受入](g9-image-generation-integration.md) に従い、
...
詳細: [installed受入](installed-reference-acceptance-20260918.md)。
...
詳細は[実装・実機記録](m4-surface-20260918.md)。
...
通常Host取消は初回CSRF403（専用child failed回収/login失効）、ヘッダー修正R2は8924 exit0/5.373秒。
両canceledだがHost理由文との差分receipt=falseを保持、完全一致としていない。
...
全test/短時間/取消/長時間診断は終端済み。接続切断はPython websockets clientであり実ブラウザ操作ではない。
ux1/3d-setup-evidence-scopeはこの証拠範囲を明確化する文書のみの補正。新test/runtime実行なし。
次: 文書補正を通常merge→署名配布/installed実ブラウザ受入。全体PARTIALを維持する。
...
base0dc95da、ux1/3d-setup-host-journal、PR456 open/mergeable、実装head43be15d。
Storeへprivate owner/child/outbox/receipt、再起動fail-closed。
...
次: PR456通常merge後、manager/requestへ正規identityとrefresh/control/outboxを接続。
再開: git fetch origin と gh pr view 456 でmerge stateを確認。実行中test/診断なし。
...
同handleで384.873秒skill、641.007秒media.capabilities実行の出力確認。
前turnは同PID確認による検証待ち、今回は最終test成功と診断commit/push/PR作成まで進捗。
