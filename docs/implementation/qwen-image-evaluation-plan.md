...
現構成はCPU互換性・RAM余裕・生成時間の条件を満たさず、製品導入を見送る。
GPU/int8で通常画像1枚は生成できたが708秒を要し、3回の確認は時間上限で未完了。
以下の容量記述は当初の見積もり。実測ピークと失敗理由は実行記録を正とする。
...
- 2026-09-22の再測定では、環境変数単独のfp32不一致が残った。
  明示hipBLASでは試したfp32/bf16は一致するが、fp16の大規模形状は不一致。
  [評価記録](qwen-image-21-evaluation-20260922.md)の設定・範囲を参照し、
  全dtypeが安全、または全Qwen演算を検証済みとは解釈しない。
