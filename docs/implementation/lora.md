...
LoRA だけ落としても絵は作れない。載せる checkpoint が手元に無ければ、
取り込む前にそう言い、一緒に取得するかを訊く。

* 手元に系統の合う checkpoint があれば、それを使う。何も追加で落とさない
* 無ければ、その系統で最もダウンロードされている checkpoint を 1 つ挙げる
  （Civitai は `baseModels` で絞れる。実測: SDXL 1.0 で Juggernaut XL 6.62GB）
* **大きさを見せてから訊く。** LoRA は 40MB 前後だが、土台は 2〜7GB ある。
  黙って始めると、40MB のつもりが 7GB 落ちてくる

承諾があって初めて土台の取得を始める。断られたら LoRA だけ取り込み、
「載せる先がまだ無い」ことを一覧に出す。
...
7. 取り込み前に土台の有無を調べ、無ければ候補と大きさを見せて確認を取る。
