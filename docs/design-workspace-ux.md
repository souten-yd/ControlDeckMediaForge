...
簡易・詳細・モバイルで同じ位置から操作でき、編集画面や接続の再開を必要としない。
終了選択は対象session IDを固定し、「新しい版として保存して終了」を主操作にする。
「変更を破棄して終了」は未保存編集が失われる説明とともに明示選択する。
準備中は「起動を取り消す」、保存/終了処理中は進行表示と二重送信防止を行う。
表示だけ閉じる操作はsession終了と区別する。要求失敗は終了成功とせず再試行可能にする。


## 複数方向の画像から3Dを作る（2026-09-23、実装目標）

「画像から3D」の最初の画像を正面として常に表示し、その下に「画像を追加」と
「別方向の画像を作る」を置く。従来の1枚でも開始できる。追加した方向ごとのカードには
実画像・正面/右/背面/左・差し替え・削除を表示する。枚数を埋めるために同じ画像を複製しない。
同じAsset/画像内容を異なる方向へ指定したら、重複しているカードを示して開始を拒否する。
Libraryと端末の写真から追加でき、320pxでは縦並び・44px以上の操作対象を使う。

簡易は実測済みの画像枚数/方向の組合せと推奨設定を選ぶ。詳細からcamera/framingへ到達可能。
固定の撮影条件を前提とする入力には、画像の向き・構図・大きさをそろえる案内を付ける。
自動で作った別方向は方向別の「候補」と表示し、見て選んでから3Dへ進む。
未採用のruntimeや未受入の視点生成を利用可能とは表示しない。

既存の参照編集を使う試作は「別方向の候補を作る（実験的）」として分ける。
右・背面・左の1〜3枚を方向別の依頼で作り、向き・体形・持ち物が変わる場合があることを示す。
元の正面と選んだ候補を並べ、「向き・形・持ち物を確認した」の選択後に、その方向へ設定できる。
成功だけで追加画像を差し替えず、3Dも開始しない。中止・一部失敗・再接続後も成功した候補を残し、
元画像ごとの保存済みbatchを検索する。通常の最近100件のJob一覧に依存させない。
元画像を切り替えたら別の元画像の候補を表示/採用しない。削除済み/同じ画素の候補は選択を拒否する。
1枚生成と手動追加は維持。追加の指示・seedは詳細に置き、320pxでも比較と明示選択を操作できる。
...
2026-09-22追補: 材質の派生画像はLibraryの画像ビューアーと詳細から元の3Dシーンへ戻れる。
詳細には元シーン名・制作元の版と「この画像で材質を変更」を表示し、既存3D素材にも
「材質を変更」の入口を置く。serverがownerと保存済みprovenance/親Asset/SceneRevisionを
照合する。Job一覧の保持件数に依存させず、普通の画像編集を重ねても親を辿れるようにする。
元の版を確認でき、編集は現在の版に対する未保存比較→明示採用で新しい版へ保存する。
元の対象/slot/UVが現行版に無ければ自動で別の場所へ貼らず、選び直しを案内する。
320pxの全画面ビューアー/詳細でも44px以上の操作面を確保し、画面移動だけでGUIを起動しない。
材質画像の生成では、未選択・未入力の内容を生成ボタンの近くに表示する。押した場合は
不足している欄を開いて移動し、対象を勝手に選ばない。実行中や対象情報の取得中は二重送信を防ぐ。
材質の画像選択は「この3Dの元画像」を既定とし、派生候補を含む関連画像、ライブラリ全体へ
切り替えられる。保存済みの来歴とsceneの権限から絞り、サムネイル・画像の役割・寸法で選べる。
未登録の埋込材質は明示操作で画像として取り出す。閲覧だけでは抽出や材質の適用をしない。

2026-09-22利用者依頼の追加: 主ナビの表示名を「3D」に統一する（既存 `/web-blender` routeは維持）。
Web Blenderは詳細内のフルGUI編集だけの名称とする。表示モードのラベルは「簡易 / 詳細」。
3Dの簡易モードは「画像から作る / 作った3Dを編集 / ファイルを取り込む」の
目的で入口を分ける。複数の生成ボタン、Web BlenderのフルGUI、内部設定を同時に前面へ並べない。
材質は「1. 貼り替える場所 → 2. AIで画像を作る、または画像を選ぶ → 3. 3Dで確認 → 採用」の順にする。
AI入力の文言は「3Dに貼ってある画像」「この画面で選んだ画像」「元画像なしで新しく作る」で統一し、
現在の3D名・対象・画像のサムネイルと説明を入力の近くへ出す。「選択画像」だけで省略しない。
簡易では色・模様、既存のUV、繰り返し、候補3枚を初期設定とする。単一の材質欄は既定を表示要約へ
畳み、対象objectを勝手に変更しない。複数slotなど場所の判定が必要な場合は簡易でも選べる。
3D生成engine/解像度/seed、材質channel/UV/wrap/normal/候補数、軽量化/rigの数値は詳細モードだけに
mountする。初期値は既存の採用済み・実測済み既定を継承し、詳細での明示変更をモード切替で消さない。
変更があれば簡易にも「変更した設定を使用中」と表示し、明示的な「推奨設定に戻す」を用意する。
処理中の進捗・中止、入力不足/設定不適合の説明はどのモードでも残す。必要なら詳細を開いて不足欄へ移動。
Web Blender・通し制作は詳細へ集約し、実行中/復旧候補がある場合は簡易でもその状態と停止・復旧へ到達できる。
画像の生成・選択だけではsceneを保存せず、比較後の明示採用で新しい版を作る。
...
2026-09-20追補: 「画像から3Dを生成」の既定はtrellis.cppまでで終える段とし、engineの選択は
詳細設定に残したまま既定選択をtrellis.cppにする。その上で「Pixal3Dで仕上げまで実行する」を
フォーム前面のチェックとして置き、1段目の所要時間・合計の目安・Pixal3Dが走る解像度を1行で示す。
Pixal3Dが採用されていない環境ではチェックを無効化し、trellis.cppまでで終わる旨を言う。
2段実行の成果は同じシーンの2つの版として保存し、1段目を捨てない。2段目だけが落ちたときは
1段目の保存を伝えたうえで仕上げが完了しなかったことを言い、成功として黙らせない。
入力画像は選択中のものを縮小表示し、共有Libraryからの選択に加えて端末の写真から直接選べる。
端末の写真はHEICを含む。端末が復号できる形式はその場でPNGへ直し、復号できない形式は原本を
そのままcoreへ送って復号させる。どちらでも失敗したときは理由を出し、無反応にしない。
利用者が入力したシーン名を写真選択で上書きしない。生成完了後はその場からシーンを開ける。

2026-09-20追補: Web BlenderのフルGUIはPC推奨のまま、携帯からも視点操作と文字入力ができるようにする。
1本指の操作を 選ぶ / 回す / ずらす / 寄る・引く から選び、選ぶ以外のときだけ望んだマウスボタンへ
変換する。選ぶではリモート操作系本来のタップ・ドラッグ・2本指スクロールをそのまま通す。
Shift / Ctrl / Alt は押したままにできる形にし、切断時は必ず離す。画面キーボードは隠し入力で出す。
numpadを持たない端末のために 正面 / 右 / 上 / 全体 を補助キーへ置く。押す的は44px以上を保つ。
モバイルを状態カードだけの `companion` に退行させない方針は変えない。
...
## 2026-09-23 ライブラリの削除・復元・完全削除

画像、3Dの各版、材質用画像は参照関係にかかわらず個別・複数選択でごみ箱へ移せる。
選択中は版を展開し、3DのGLB/Blenderは同じ版の1組だけを対象にする。
確認画面にserverが確定した対象名・版番号・実ファイル件数を表示する。
入力画像、別の版、派生3Dを暗黙に含めない。現在の版を削除すると残る最新の版を使用する。
ごみ箱では「復元」と「完全削除」を提供。前者は実ファイルを保持、後者は復元不可として
別の確認を必須にする。作成記録は削除済み表示で残し、削除した画像が既に3Dに埋め込まれた
部分は残ると説明する。選択数上限100、二重送信防止、具体的な失敗理由、再試行を扱う。
簡易/詳細・日英・320pxで同操作を提供し、状態はserverへ保存する。

ごみ箱には「ごみ箱を空にする」を置く。読み込み済みページだけではなく、サーバ側で
このアカウントが削除できる全対象を確定し、件数と3Dの版を確認して完全削除する。
確認中に対象が変わった場合は再確認する。一度の上限は5,000 Asset。
## 2026-09-23 複数面の手動入力

3Dの「画像から作る」に、採用済みの複数面capabilityがある場合だけ
「追加する方向」で右側面・背面・左側面の未追加の方向を選び、「この方向を追加」する。
元の入力は正面で、追加は最大3枚。正面＋左側面、正面＋背面、正面＋背面＋左側面など
必要な組合せで生成できる。各カードに方向名・実画像・Library選択・端末からの追加・
そのカードだけを外す操作を置く。選んだカードの画像が揃えば開始可能で、
全追加カードを外せば従来の1枚生成へ戻る。AI候補の採用もその方向だけを追加する。
別Asset IDでも同じ画素はbackendが拒否する。画像の独立cropを暗黙に行わず、
共通canvas・背景透過・共通の撮影条件が必要なことを操作前に示す。

簡易モードはPixal3D・1024・推奨turntable cameraを選ぶ。詳細モードでは
画角・距離・仰角・倍率を変更可能にする。自動校正や別方向画像の完成を意味しない。
同じ正面を別エンジンへ渡す従来の2段処理は「同じ画像からPixal3D版も作る」と表示し、
複数面生成と区別する。320px幅でも画像/選択/取消を操作でき、追加操作は44px以上。
