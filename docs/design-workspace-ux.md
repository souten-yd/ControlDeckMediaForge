...
2026-09-12追補: Web Blenderの先頭には、シーン未選択でも見える編集入口を置く。
選択対象を明記し、未選択時は編集を無効化して一覧からの選択を案内する。
シーンがない場合の取り込み・OpenCode制作後の一覧更新と、Blender設定への導線を示す。
画面移動だけでGUIを起動せず、既存の環境・処理中・単一編集セッションの制約を維持する。
この入口の改善を、新規シーン作成UIやOS隔離設定の自動導入の完成とは扱わない。
2026-09-19追補: 採用済みの `3d.image_to_3d` capability がある場合、同じ画面に
「画像から3Dを生成（実験的）」を追加する。入力は共有Libraryの画像を選び、シーン名を付ける。
シード・採用環境で実測済みの生成解像度は詳細設定へ置く。
生成エンジンも詳細設定へ置き、capabilityのengine別状態から自動/trellis.cpp/Pixal3Dの選択肢を
導出する。未準備の選択肢は理由付きで無効化し、選択engineの実測解像度だけを使う。
利用者が選択したengine/解像度が更新で使えなくなったときは入力を保持して送信を止め、選び直しを
案内する。別engineや解像度へ黙って切り替えない。CPU前処理は「入力画像を準備しています」とし、
GPU待ち・3D生成・検証保存と区別する。生成状況・中止・接続失敗時の更新を
同じフォームに表示し、完了後は既存シーン一覧・Libraryへ接続する。単体表示ではControlDeckからの
操作を案内する。未採用環境の生成ボタンを出さず、実験的な形状・見えない面の確認を促す。
重量モデルの導入・同意・採用完了をフォーム実装だけで成立させない。
...
| 画像 / 動画の作成切替 | L1 | ヘッダーの表示モードのすぐ左に絵の 2 択で常駐する。頁の上部を 1 行使うほどの選択ではない。絵は文字を持てないので、試験中は印と `aria-label` / `title` の両方で伝える。動画はcapabilityが使える場合だけ操作可能 |
...
| 参照ごとの役割・強さ | L3 | roleはadapter対応分のみ。strength未対応は理由付き無効 |
...
| 動画の画質（標準 / 横長 / 正方形） | L1 | 動画のときだけ出る。数値ではなく意味で選ばせる。面積は注意機構に二乗で効くので、選択肢を絞って目安時間を添える |
| 動画の長さ（2 / 3 / 5 秒） | L1 | 同上。フレーム数に比例して時間が伸びる |
| 動画の目安時間 | L1 | 実測（512x320・33 フレーム・30 歩で 144.6 秒）からの外挿。13 分かかるものを押してから知らせない。初回のモデル読み込みは別途明示する |
| 動画の歩数 / ガイダンス / フレーム数 / fps / 打ち消し語 | L3 | 簡易の選択を奪わない。指定したものだけを送り、残りは選ばれたモデルの実測既定で埋まる |
...
| 動画作成 | L1切替 | G7契約を再利用。unavailable時は実行を無効化して理由を1行表示 |
| 3D生成 | 非表示 | G9未実装。空タブを作らない |
...
種別フィルタは媒体の表示・操作が異なるときに選択範囲を狭めるためのものとする。
生成/編集/取り込みという来歴はカードと詳細で常に表示するが、それだけを理由に見出しを
占有するフィルタへ戻さない。3Dカードは一覧でGLB本体を取得せず、検証済みthumbnailまたは
軽量な3D placeholderだけを表示し、カードを開いた時点で初めてviewerとGLB bytesを遅延取得する。

2026-09-06利用者確認: Libraryの3D表示は対象の±X/±Y/±Z回転と拡大・縮小を必須とする。
6方向の固定視点ではなく、各軸の正負方向への回転。表示だけを変え、元asset/sceneは変更しない。
ワイヤー・アニメーション等は追加操作であり必須受入にしない。制作画面の材質比較・採用は維持する。
ゲーム制作の追加animationプレビューでは、動作名/秒数の選択、再生/一時停止、先頭戻し、
0.25/0.5/1/2倍速を表示する。選んだ一つだけを繰り返し再生し、元素材へ保存しない。
clipのない素材には操作を出さず、日英/320pxでも選択・操作・停止を維持する。
...
画像 / 動画切替はCreateの先頭で全幅表示し、各44px以上。ヘッダへ詰め込まない
動画runtimeがunavailableでも媒体切替は開けるが、実行をdisabledにして理由と設定への出口を出す
...
Createの画像 / 動画切替は、利用者が媒体を選ぶ安定したL1操作として常に描画する。
`video.text_to_video` / `video.image_to_video` が両方unavailableでも動画フォームは確認できるが、
...
C5 複数枚生成時は候補ストリップで全候補を見比べられる
     pose / scene / composition の意図的差分は親セットとして進捗を復元し、
     成功した子の素材は一部失敗・中止でも候補から失わない
     詳細モードの状況では親セットから子 job ID を展開できる
     Poster / Character Sheet は別画面を作らず Domain 選択後にタイトル・説明を開示し、
     2〜4 child shot の完了後は Composer の最終1枚を同じresult/viewerへ表示する
     タイトル・説明の再編集はshot jobを再実行しない
...
13-10 動画タブを今から置く（disabled 表示）
     2026-08-26に判断を更新。G7で公開operationと候補管理まで実装され、利用者からCreate上部の
     画像 / 動画切替が明示要求されたため、独立タブではなく同一Create内の媒体切替として採用する。
     unavailable時もintent/入力画像を確認できるが、実行はdisabled + 理由 + 設定への出口とする。
...
A10 320pxでもCreateの先頭で画像 / 動画を切替でき、使えない動画を実行できるようには見せない
```
