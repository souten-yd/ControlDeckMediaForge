...
| GOAL-02 | ブラウザだけで3Dを確認 | GLB対象の±X/±Y/±Z回転と拡大・縮小を確認（2026-09-06利用者確認） |
...
2026-09-20追補: 入力補助は「押せる」だけでなく「視点が動かせる」ところまで持つ。
リモート操作系のジェスチャは1本指＝左ボタンで、Blenderの視点操作に要る中ボタンの
ドラッグを作れない。1本指の操作を 選ぶ / 回す / ずらす / 寄る・引く から選べるようにし、
選ぶ以外のときだけ望んだマウスボタン・ホイールへ変換する。選ぶでは横取りせず、
リモート操作系本来のタップ・ドラッグ・2本指スクロールをそのまま通す。
Shift / Ctrl / Alt は押したままにできる形にし、切断時は必ず離す。画面キーボードは
canvasでは出ないため隠し入力で出す。numpadを持たない端末のために 正面 / 右 / 上 / 全体 を
補助キーへ置く。押す的は44px以上を保つ。

2026-09-20追補: 「画像から3Dを生成」は engine の三択ではなく「既定の段 + 足す段」で見せる。
既定はtrellis.cppまで、チェックでPixal3Dまで。所要時間の目安と、足す段が走る解像度を示す。
2段実行の成果は同じシーンの2つの版として残し、1段目を捨てない。入力画像は端末の写真からも
選べる（HEICを含む）。この時点の採用中CLI/workerは1枚入力のみ。

2026-09-23追補（実装目標、複数面runtime/UIは未採用）:
従来の1枚入力に、別方向の画像を最大3枚追加する流れを加える。正面の画像を常に見せ、
追加した各画像にも方向・サムネイル・差し替え/削除を表示する。Libraryと端末写真の両方を使える。
同一Assetや同じ画像内容を複数方向へ割り当てる入力は開始前に拒否する。追加画像を無視して
1枚生成に落とすことも、正面画像を複製して枚数を埋めることもしない。
簡易モードは測定済みの組合せと推奨値を使い、詳細でcamera/framingを変更できる。
角度ラベルだけで任意の写真のcameraが校正済みになるとは扱わない。
「別方向の画像を作る」は選んだ正面を編集元として候補を生成し、各方向の候補を見て採用する。
顔/部位の重複や構図が崩れた候補を自動で3Dへ送らない。手動入力と1枚生成は独立して維持。
320pxでは方向ごとのカードを縦に並べ、追加/差し替え/削除/生成/停止を44px以上で操作する。
