...
`POST /api/v1/jobs` accepts [`schemas/job-request.json`](../schemas/job-request.json). G0 executes `image.generate` through a deterministic fake worker. Other operation names are reserved by the public contract and fail with `capability_unavailable` until their goal is delivered.
...
`GET /api/v1/capabilities` reports capability state as `available`, `unavailable`, or later `experimental`. G0 reports `image.text_to_image` as available with `implementation=fake` and `confidence=low`; it does not claim a real model is installed.
