...
参照画像、採用済み版、今回の候補を別々に識別する。画像のrevisionが今回のJob結果と
一致しなければ、新しい形を確認したとは扱わない。画角・投影・姿勢・照明が異なる場合は
比較不能と記録し、改善の点数を作らない。対象IDはsnapshotまたは明示的な画像対応表から
取得する。画像だけで特定できない部品は「対象不明」とし、そのまま編集命令を出さない。

依頼した変更に加え、以前に採用した箇所の悪化も調べる。「欠陥なし」という応答でも、
背面が渡されていなければ背面は未観測のままである。骨や裏面の検査結果を画像観測と混ぜない。

### アセット別の確認

`authoring_guidance.asset_family_checks`を対象に応じて読む。これは対応機能の宣言ではない。
人型は輪郭と顔・手、髪は根元・分け目・頭頂と後頭部・先端、服と防具は厚み・重ね順・
留め具・関節の逃げを確認する。クリーチャーは対象の身体構成と接地、武器と道具は接合と
握る空間、乗り物は軸と可動部、環境物はグリッドと継ぎ目・裏面・遠近での反復を確認する。
静止画で成立しても、曲げ姿勢・export後・ゲームでの評価を省略しない。
