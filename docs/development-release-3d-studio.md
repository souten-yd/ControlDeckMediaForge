...
### 検証用プロジェクトの後片付け（2026-09-10利用者指示）

OpenCodeの納品確認用に作るMF3DSプロジェクトは常用プロジェクトではない。
検証終了後、成功・失敗と再開に必要な証跡を記録し、稼働Job/processが使っていないことを確認して
プロジェクト一覧から回収する。次の検証に必要な場合だけ対象・理由・回収条件を引き継ぐ。
名前のprefixだけで一括削除しない。作成時の対象一覧・内容・利用者による追加変更を照合し、
利用者の制作物・MediaForgeの元Asset/scene/historyを巻き込まない。
復元可能な退避を使った場合は、完全削除と呼ばず退避先を記録して利用者に伝える。
過去の納品先pathは回収後には存在しないため、現在の実ファイルとして再提示しない。

2026-09-12利用者追加指示: 検証用data、古い一時backup、配布コピーを無期限に蓄積しない。
結果・失敗条件・再開条件はPRとリポジトリへ記録し、終了した検証の不要な実体は使用中参照と
正式制作物からの分離を確認して削除する。/data1tb直下へmf-*を大量作成する運用は繰り返さず、
data_dir配下の管理された作業領域を使う。保留が必要なら対象・必要理由・回収条件を明記する。
PRの要約は生の画像/DBそのものではないため、削除後にraw evidenceも保存済みと説明しない。
正式制作物・利用者backup・管理対象runtime/正式rollback版の削除権限を、この検証cleanupから拡大しない。
