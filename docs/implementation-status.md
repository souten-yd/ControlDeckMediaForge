...
## 2026-09-11 batchの件と件の間でleaseを持ち回る（0.28.78）
...
持ち回る。linger（warm_linger_sec、既定0）と同じ判断だが、引き金が時計ではなく宣言なので
既定のままで効く。同じhost jobに属し、同じmodelをgpu0に載せたままのときに限る。資格の
更新は件ごとの受付を通らなくなるので、引き継ぎのところで明示して行う。workerを降ろすときに
必ず返す——降ろすのが先、返すのが後で、返しただけではVRAMは空かない。
