...
## 残課題: batchの途中で言語モデルが降ろされる

0.28.77の導入受入で見つけた。1枚ずつの生成は同居したまま通る（19秒/15秒、VRAM 21.1GiB
のまま）。4枚のbatchは4枚とも成功するが、途中で言語モデルが降ろされる。

  0.0秒  VRAM 21.2GiB  LLM生存
 39.0秒  VRAM 28.2GiB  LLM生存   ← 3枚目まで同居できている
 50.6秒  VRAM  2.0GiB  LLM不在   ← ここで降ろされた
 73.5秒  4枚とも成功

申告は1件あたり山6.6GiB＋余白2.2GiB＝8.8GiB。言語モデルの21.1GiBと足して29.9GiBで、
31.86GiBには収まる計算だが、余りは1.96GiBしかない。brokerの受付はadmitted_used =
max(observed, fixed+lease)で見るので、前の1枚のVRAMが返る前に次の受付が走ると観測値が
効いて入らなくなり、broker が言語モデルを明示解放する。降ろすのは「戻ってくる退避」で、
KVを預けるため復帰は11.3秒＋初回応答0.4秒。読み直し165秒は起きない。

再現は時刻依存で、13秒で降りることも50秒で降りることもある。1枚ずつなら起きない。

根は「batchがN件のjobをそれぞれ独立に受け付けさせている」ことで、下のbatchの符号化を
まとめる課題と同じ形である。batch全体で1つのleaseを取れば、受付は1度で済む。
