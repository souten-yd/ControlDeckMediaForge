...
## 2026-09-11 batchの件と件の間でleaseを持ち回る（0.28.78〜0.28.79）

0.28.77の導入受入で、4枚のbatchが3枚目まで同居できてそこで言語モデルが降ろされた。
1枚ずつなら起きない。原因を実測で詰めた。

画像生成が実際に要るVRAMは枚数によらず一定である（同じpipelineで1〜4枚、枠8.8GiB）:
...
allocatedが真に要る量で、cpu_offloadなので部品は毎回RAMと往復し積み上がらない。
reservedが8.50で頭打ちなのはworkerへ渡している枠が8.8GiBだから。driverの21.27GiBは
初回生成中だけ現れるtorch外の作業領域で、HIPの起動と小さな行列積・畳み込みだけなら
0.45GiBなので模型の最初の推論で確保される。要件ではない——言語モデルが21.1GiB載って
空きが10.7GiBしかなくても4枚とも通る。

降ろしているのはControlDeckである。MediaForgeはcompute_mode: shared-safeで要求し、
ai/releaseは呼んでいない（以前呼んでいたのは外してある）。経路は
resources/broker.py の _ask_for_room → provider.step_aside →
llama.release_loaded_llms(preserve_state=True) → stop_instance で、systemdの記録も
Result=success/ExecMainStatus=0。KVを預けるので復帰は11.3秒＋初回応答0.4秒。

引き金は「件ごとにleaseを返して取り直す」ことだった。返した瞬間、brokerから見た空きは
変わらない（workerがVRAMを抱えたまま）のに、次の件が改めて8.8GiBを求める。申告8.8GiBは
実測6.15GiBの1.4倍あり、brokerが見るVRAMは2秒ごとの標本で最大4秒古く、前の1枚の山が
まだ見えている間に次の受付が走る。置けないと判じたbrokerがstep_asideを呼ぶ。

0.28.78で、続きが来ると宣言されている間（keep_worker_warm）はleaseを返さずに次の件へ
持ち回るようにした。linger（warm_linger_sec、既定0）と同じ判断だが、引き金が時計ではなく
宣言なので既定のままで効く。同じhost jobに属し、同じmodelをgpu0に載せたままのときに限る。
資格の更新は件ごとの受付を通らなくなるので、引き継ぎのところで明示して行う。workerを
降ろすときに必ず返す——降ろすのが先、返すのが後で、返しただけではVRAMは空かない。

0.28.78は実機で効かず、追い出しが50.6秒から22.5秒へ早まった。引き継ぎの判断に_warm_model
を使ったのが誤りで、あれは「重みをVRAMに抱えているか」であり、部分退避（cpu_offload）では
毎回Noneになる。実運用のFluxはcpu_offloadなので一度も引き継がず、持ち回ったleaseを抱えた
まま新しいleaseも取りに行き、同じdeviceに2つぶんを求めていた。1つぶんより入らない。

0.28.79で二つ直した。lease側の問いは「同じworkerが同じmodelを次も走らせるか」なので
_carried_modelとして別に持つ。そして引き継がないと決めたときは必ず返す——抱えたまま次を
求めないことが要点である。carried/adopted/not adoptedをログに出すようにして、実機で
判断がどう転んだかを後から読めるようにした。
