...
詳細は[方向選択](implementation/multiview-direction-choice-20260923.md)。物理スマホ/任意角度/自動方向候補の品質は未受入。
...
[詳細](implementation/view-candidates-20260923.md)。
...
対象84tests通過。最終全2517passed/3skipped/3warnings/352.92秒、exit0。installed/署名配布は未実施。[詳細](implementation/reference-edit-admission-20260923.md)。
...
自動視点画像、人物/非対称/取消、installed/MCP/mobileのMVは未受入。[詳細](implementation/multiview-20260923.md)。
...
main6383fdcをmerge、製品差分なし。詳細[MV実測](implementation/multiview-20260923.md)。
...
installed新経路はNOT TESTED。モデル追加取得0。[詳細](implementation/agent-failure-reference-20260923.md)。
...
installed新版/形式省略MCPはNOT TESTED。[詳細](implementation/pack-output-default-20260923.md)。
...
両方12bones/1walk clip/重み欠損0、元画像・元revision保持。最終商店街、物理スマホはNOT TESTED。[詳細](implementation/rig-surface-20260923.md)。
...
全2496tests/3warnings/313.80秒通過。追加モデル取得0。installed/MCP再実行と最終商店街はNOT TESTED。[詳細](implementation/rig-surface-20260923.md)。
...
詳細[GLB設定公開](implementation/compile-options-discovery-20260923.md)。
...
削除済みobjectの画像から移動直後はtarget空/適用disabledだったが、材質情報の再取得で
唯一の残存objectへ自動選択され、適用が有効になった。単一対象の自動選択を除去し、
明示選択または有効な元対象の復元だけで割り当てる。保存の自動実行は元から無い。
0.33.3はローカルへ適用せず、修正版0.33.4で通常更新とinstalled受入を行う。
修正後は再取得後も空/disabledを維持。./mf.sh test: 2410 passed / 3 warnings / 276.06秒。
実測: maintenance/texture-scene-links-20260922/stale-target-{before,after}.json。
...
詳細: [Qwen Vulkan実測](implementation/qwen-image-21-vulkan-20260922.md)。元checkout `feat/g9-image-to-3d`は変更していない。
...
詳細: [Qwen評価記録](implementation/qwen-image-21-evaluation-20260922.md)。
...
詳細: [Qwen評価準備と残ゲート](implementation/qwen-image-21-evaluation-20260922.md)。
...
低ポリ側の UV に面積ゼロの三角形が **1 個**（約 24,900 三角形中）あった。
worker と backend の両方が「退化がゼロでなければ断る」だった。

面を削れば UV に面積ゼロの三角形が少数できるのは普通のことで、その三角形に
テクセルが乗らないだけである。他の面には影響しない。1/24,900 で断るのは、
守ろうとしている「UV が壊れている」という状態とは違う。

許容を入れた（**1000 分の 1 か 64 の大きい方**）。ほとんどが潰れている UV は
今までどおり断る。退化数は結果の `uv` に出るので、黙って通すことにはならない。

### 実機で焼けた

```
高 58,203 頂点 → 低 12,463 頂点
normal 680,481 B / ao 321,903 B
どちらも 262,144 画素すべて被覆（512x512 全面）
```
...
法線マップに高周波の斑点が出ている。cage の設定がこのモデルに合っていない。
製品自身も `ray_hit_coverage: not_measured` / `uv_overlap: not_checked` /
`surface_approval: not_granted` と返しており、良いとは主張していない。

**経路が通ったところまでが今回の成果**で、cage と解像度の追い込みは別作業である。

## 2026-09-22 ローポリ化の割合が元に対する値になる（0.32.0）

指摘を取り違えていた。「段階を細かく調整」は**割合の刻み**の話であって、
軽量化を重ね掛けする話ではなかった。

### 割合が嘘をついていた

削る相手が**現在の head** だった。そのため 60% のあとに 55% を選ぶと、
55% の対象は 60% 版であり、結果は元の 33% になる。画面が言っている割合と
出来上がるものが合っていなかった。

編集は head の上にしか積めない設計（`scene_revision_conflict`）なので、
サーバ側で**先に元の版を head へ戻してから削る**ようにした。元の版は版として
残り続けるので、何も失われない。

実機で測った。

```
60% →  78,392 面   元に対して  60.0%
30% →  39,195 面   元に対して  30.0%

前回に重ねていたら 30% は約 23,518 面（元の 18%）になっていた
```

### 刻みと、何を削るのか

- 割合の刻みを 5% → **1%**（範囲 5〜95%）。5% 刻みでは狙った軽さに寄せられない
- 画面に「元にする版」を出した。既定は最初の版。Pixal3D の追加段など
  作り直した版を元にしたいこともあるので選べる。**何を削っているのかが見える**

### `mesh.weld` の allow_noop について

重ね掛けのために足した形になっていたが、そちらは不要な要件だった。残す理由は
別にある。**手で組んだシーンのメッシュは最初から繋がっており、weld が何も
統合しないのが正常**である。単独で頼まれた `mesh.weld` は今までどおり、
何も繋がなければ失敗する（相手を間違えている合図になる）。

## 2026-09-21 通しフローを画面からも（0.31.1）

MCP からは通るようになったが、画面からは触れなかった。3D の画面に
「通しで作る」を足した。

画面が決めるのは 2 つだけである。

- 何から始めるか（上で選んだ画像）
- どこまでやるか（段ごとに止めるか／骨を入れるか）

**段の並びも recipe も画面からは送らない。** 契約テストで
`operations` `mesh.weld` `rig.auto` `stages:` の文字が画面側に出ないことを固定した。
画面に任意の recipe を撃つ力を与えると、サーバ側の順番が意味を失う。
...
画像        完了
3D にする   完了
骨を入れる  確認待ち     ← ここで「次へ進む」が出る
書き出す    待ち
```

進むのは poll したときだけなので、画面を閉じている間に勝手に進むことはない。
「次へ進む」は確認待ちのときだけ出て、走っている間は「通しで作る」が引っ込む。

### `<section>` を 1 つ足しただけで契約テストが落ちた

既存のテストが `id="scene-studio"` から**最初の `</section>` まで**を studio の
範囲として切り出していた。中に新しい `<section>` を入れると範囲がそこで切れ、
`scene-list` が見つからなくなる。`scene-blender-entry` と同じく `div` + `role=region`
にして解いた。範囲を閉じ括弧で決めている検査は、入れ子に弱い。

## 2026-09-21 `rig.auto` を 6 脚以外で確かめた

0.29.1 では 6 脚しか試していなかった。脚の本数は数えて決めているという主張を、
実際に別の本数で確かめた。

### 数は正しく数える

密なメッシュを 2 / 4 / 6 脚で作って測らせた。

| 脚 | 検出 | 方位（度） |
|---:|---:|---|
| 2 | 2 | -90.0, 90.0 |
| 4 | 4 | -143.8, -36.2, 36.2, 143.8 |
| 6 | 6 | -157.1, -90.0, -22.9, 22.9, 90.0, 157.1 |

製品経由（`media.scene.rig`）でも骨入れまで通った。

```
4脚: 骨 13 本  root / extra1-4 / leg1-4_upper,lower  walk 24f
2脚: 骨  7 本  root / extra1-2 / leg1-2_upper,lower  walk 24f
```

### 分かった制約: 測るのは頂点であって面ではない

最初の 4 脚モデル（角柱と円柱をそのまま繋いだもの、792 頂点）は
`no horizontal band near the base carries enough geometry to find limbs` で断られた。
調べると**帯に頂点が 0 個**だった。円柱は端面にしか頂点が無く、脚の途中には何も無い。

`rig.auto` は**密に割られた表面**を前提にしている。生成した 3D はそうなっている
（実測 127,796 頂点）ので本来の用途では問題ないが、手で置いた原始形状には
当たらないことがある。細かく割った同じ形（7,464 頂点）では正しく測れた。

### 分かった弱点: 突起の検出は疎なメッシュで当てにならない

合成モデルでは `extras` が脚と同じ数だけ出た。実物のロボットでは触角 4 本を
正しく見つけているので実害は出ていないが、塊に分ける升目の大きさを
**模型全体の広がり**から決めているのが原因である。頂点の間隔が升目より広いと、
繋がっているはずの輪郭がばらばらの点として数えられる。

升目を頂点間隔から決めるべきだが、それは測り直しの設計そのものなので、
ここでは**観測した事実として記録するに留める**。骨は余分に増えるだけで、
bone heat は全頂点を解いており、動きも付く。
...
**書いた動きが意図どおりに見えるかを、書いた側が確かめられない**こと。

### 観察は姿勢を描けなかった

`worker_packs/blender/scene_observation.py` は `scene.frame_set(0)` 固定で、
結果も `"frame": 0` と言っていた。どのフレームを頼んでも**レスト姿勢しか返らない**。
クリップを付けても、観察で見えるのは立ち姿だけだった。

### 入れたもの

`ObservationSpec` に `clip_id` と `frames` を足した。クリップは muted な NLA track に
仕舞われた action として保存されているので、名前で 1 つ選んで骨へ割り当ててから
各フレームを描く。

歯止め:

- `clip_id` と `frames` は両方揃って初めて受ける。フレームだけ動かしても姿勢は
  変わらないので、「動いて見えない」の原因を探させないために断る
- `frames` は重複なし・昇順・0〜600、最大 8
- **views × frames ≤ 8**。CPU のパストレースなので、フィルムストリップは頼めない
- 仕舞ってある track は mute のまま。鳴ると割り当てた action と二重に効く

### fail-closed が 1 回効いた

`ObservationSpec` に足しただけでは動かなかった。worker 側にも独自の
`validate_spec` があり、キーの集合が一致しなければ拒む。実機で
`scene_observation_failed` になり、手元で同じことを再現して
`invalid observation fields` を見つけた。**型を 1 か所足しただけで通ってしまう
作りではなかった**ということで、これは正しい挙動である。

### 実機で見た

`R05 WEAVER 自動リグ` の版 2（`rig.auto` が測って組んだ 17 骨 + walk）を
側面 256px clay で描いた。

```
frames [0, 6, 12, 18] / clip_id walk
  side f0   e0ac1ed1681961fc
  side f6   c82468ca176bc48d
  side f12  d7e75f46ace58923
  side f18  71395866aac08ae0
別々の絵: 4 / 4
```

絵を並べると脚が前後に振れているのが見える。画素で測ると:

```
f0  vs f12 :  0 %  ← 同じ姿勢。残りは 16 サンプルの粒子
f6  vs f18 : 21 %  ← 逆位相なので大きく違う
```

0.29.1 で「フレーム 12 が基本姿勢に戻るのは逆位相の性質で正しい」と書いた説明が、
**絵でも裏付けられた**。数字だけの主張だったものが見えるようになった。

### これで輪が閉じる

```
media.scene.snapshot   骨を見る（名前・親子・回す向き）
media.scene.edit       任意のクリップを書く
media.scene.observe    その姿勢を描く  ← ここが繋がった
```

書く → 描く → 見る → 直す。LLM にアニメを任せるのに要る材料は揃った。

## 2026-09-21 骨を見せる `skeletons` facts（0.29.2〜0.29.3）

「アニメーションは、LLM に任せると、規定が無くても自在に決められないか？」への答え。
**その通りで、しかも規定を増やすのではなく、既にある自由を使えるようにする話だった。**

### アニメの語彙は最初から型なしだった

`animation.clip` が受けるもの:

```
tracks   最大 128（骨ごと）      keys   1トラック最大 256
rotation XYZ 各 ±180 度          translation 任意（±100m）
frames   1〜600、最長 120 秒
```

**任意のキーフレーム列**であって、「歩く」「走る」といったパターンの集合ではない。
`rig.auto` の walk はただの既定値である。足りていなかったのは語彙ではなく、
**書く側が骨を見られないこと**だった。版が返していたのは件数だけで
（`objects: 2` のような数）、骨の名前も親子も、どの軸を回すとどちらへ曲がるかも
出ていなかった。さらに `mesh_geometry` は 16,384 頂点までの上限付きなので、
35,338 頂点のリグ済みモデルではその件数すら返っていなかった。

### 出したもの

骨 1 本ごとに id・親・子・レストの head/tail・長さ・ローカル軸、そして
`rotation_response`。これは「その軸まわりに正の回転を掛けたとき、先端が現実に
どちらへどれだけ動くか」を worker が実際に回転行列を当てて測った値である。
骨はリグあたり 128 本までなので、mesh_geometry のような量の上限は要らない。

### 単位ベクトルにしてはいけなかった（0.29.3 で修正）

最初の実装は応答を単位化していた。実機の出力を見ると、**骨に沿う軸**
（回しても先端は動かないはず）がこう返っていた。

```
axis_y      [-0.0446, -0.9629,  0.2662]   骨に沿う向き
response.y  [ 0.0,     0.0,    -1.0   ]   ← 「真下へ動く」と読める嘘
```

丸め誤差しか残っていない変位を単位化して、自信ありげな向きを作っていた。
読む側はそれを本物の「真下へ動く軸」と受け取る。**当て推量を消すための facts が、
新しい形の当て推量を持ち込んでいた。**

直し方は単位化をやめて長さを残すこと。長さ 1 が「その軸で動かせる最大」
（長さ L の骨を角度 a 回すと先端は最大 2L·sin(a/2) 動く。これを 1 とする）、
0 が「この軸は効かない」。0.02 未満はゼロと言い切る。修正後の実測:

```
response.x  [-0.0565,  0.9736, -0.2213]   長さ 1.00
response.y  [ 0.0,     0.0,     0.0   ]   長さ 0.00 ← 正しい
response.z  [-0.9944, -0.0330,  0.1005]   長さ 1.00
```

### 実機で「パターン無しで動きを作れる」ことを確かめた

`media.scene.snapshot` で骨格を読み、`rotation_response` を見て軸を選ぶだけの
コードで、**`rig.auto` が絶対に作らない動き**（触角を振る）を組んで通した。

```
rig rig bones 17
authored 4 tracks from the facts, no pattern involved
status succeeded
clips  antenna_wave (30f) / walk (24f)
```

これで「脚が 6 本ある形」という `rig.auto` の前提も外れる。骨さえあれば形を問わない。

### まだ無いもの

**確認の輪。** 書いた動きが意図どおりに見えるかを、書いた側が見られない。
`media.scene.observe` が版を画像に描けるので「書く→描かせる→見る→直す」は
作れるが、まだ繋いでいない。

## 2026-09-21 測って骨を置く `rig.auto`（0.29.1）

「ローカル LLM でモデルに応じた骨組みとアニメーションを設定してほしい」「画像→3D→
アニメを通しで使いたい」への 1 歩目。**骨の置き方そのものを製品に入れた。**

0.28.95〜0.28.98 で通した歩行リグは、脚の方位も触角の位置も手で測って座標を書いた
（`legs-clean.json` / `extras.json` を外で作った）。それでは次のモデルで同じことが
できない。測る部分を Blender worker の中へ移し、typed operation にした。

### 測り方

`worker_packs/blender/rig_auto.py`。上は Blender の +Z（取り込みが glTF の +Y から
変換済み）。

1. 下から 12% の高さに厚さ 5% の水平帯を取り、XY を格子に刻んで繋がった升を塊にする
   → 塊の数が脚の数
2. 塊の重心の方位で模型を扇形に分け、各扇形の頂点を高さ順に並べて 3 か所の重心を取る
   → 下から 足 / 膝 / 付け根
3. 胴の太さは上半分の点の半径の中央値。胴の下端から上端を貫く root を置く
   （bone heat はどの骨からも遠い頂点を解けないので、胴にも骨が要る）
4. 86% の高さの帯で、胴の半径より外に出ている塊 → 触角などの突起。1 本ずつ骨を置く
5. 脚は隣どうしを半周期ずらす。6 本なら三脚歩行、4 本なら斜対歩になる

出すのは `armature.create` / `skin.bind_auto` / `animation.clip` の operation 辞書
そのもので、手で書いたときと同じ関数・同じ検査を通る。新しい低水準の仕組みは無い。

### 実機で手測りと突き合わせた

同じ GLB を入れ、正規の MCP 道具 `media.scene.rig` から走らせた。

| | 手で測った版（0.28.97） | `rig.auto`（0.29.1） |
|---|---:|---:|
| 頂点 | 35,338 | 35,338 |
| 三角形 | 78,392 | 78,392 |
| 骨 | 17 | 17 |
| joints | root / 触角4 / 脚6×2 | 同じ |
| クリップ | walk 24f @24fps | walk 24f @24fps |

書き出した GLB を読み直して測った結果:

```
bones 17 / vertex_groups 17 / unweighted 0
lowest_foot_z_by_frame  0:-0.2561  6:-0.2705  12:-0.2561  18:-0.2622
frames_with_pose_change 2 / 3
```

`unweighted 0` は bone heat が全頂点を解いたということ。フレーム 12 が基本姿勢に
戻るのは正しい。逆位相の 2 組は同じ瞬間にゼロ交差するので、周期のちょうど半分では
全脚が中立になる。

### 並びは 1 か所に持つ

`SceneWorkspace.rig_recipe()` が「繋ぐ→落とす→測って組む」を組み立て、画面の
`scenes.rig` と MCP の `media.scene.rig` が同じものを呼ぶ。別々に組むと、どちらかが
`mesh.weld` を落とす。落としたときの症状は「骨が入らない」ではなく
**「重みが 1 つも付かない」**なので、原因に辿り着くのに時間がかかる。

### 使える形の範囲

脚が 2 本以上見つかる形だけ。見つからなければ骨を捏造せずに失敗する。人型や
手で決めたい骨組みは、従来どおり `media.scene.edit` の `armature.create` を使う。
歩行の振れ幅の軸は `rig.auto` が組んだ upper / lower の向きに合わせてあるので、
手で置いた骨にこのクリップは当たらない。

## 2026-09-21 同じシーンの版を 1 枚にまとめる（0.29.0）

「同系列のモデルで、修正されると同じモデルが連番でライブラリに表示されるが、
表示上まとめられないか？」と「シーンファイルって一体なに？意味あるのか？」への答え。

### シーンファイルとは何か

`.blend`（`application/x-blender`）のことで、**版の土台**である。1 つの版は資産を
2 つ登録する。

| 資産 | 役 | 使い道 |
|---|---|---|
| `.blend` | `source_asset_id` | 次の編集がここから積み上がる。`scenes.from_glb` / `scene.edit` の入力 |
| `.glb` | `preview_asset_id` | 見るため・書き出すため |

だから編集を 1 回重ねるごとに一覧の行が 2 つ増える。消すと同系列の編集が続けられ
なくなるので意味はあるが、**一覧に 1 行ずつ並べる必要は無い。**

### 実機で数えた

```
シーン 85 / 版 227  →  ライブラリの行 454（.blend 227 + GLB 227）
全体 1421 行 → 1052 枚（属する 454 行が 85 枚に、残り 967 行はそのまま）
```

### 束ねる鍵は scene_id であって名前ではない

最初に名前で数えたら「R05 WEAVER 歩行リグ 12 行」が出た。中身を見ると
`(1,'preview')` が 5 つ並んでいて、**同じ名前の別シーンが 5 つ混ざっていた**
（骨入れの失敗試行がそれぞれシーンを作っていた）。名前で束ねると無関係なものが
1 枚になる。`scene_id` で数え直すと、正しく別カードのまま 13 枚になった。

### server は事実だけ、束ねるのは画面

`store.scene_membership(asset_ids)` が 1 往復で「どの資産がどのシーンの何版目か」を
返し、`library.page` が行それぞれに `scene` を載せる。畳むのは画面である。理由は 2 つ。

- 版は頁をまたいで届く。server 側で畳むと、同じシーンの続きが次の頁に来たとき
  既に出したカードへ足せない。
- 頁送りの錨は資産の行のままにしておきたい（`next_before` は読んだ最古の行であって、
  絞り込みを生き延びた行ではない）。

### 画面の決め事

- 代表は「いちばん新しい版の、見るための GLB」。名前はシーンの名前を出す
  （`Export validated scene preview` ではなく `R05 WEAVER 歩行リグ`）
- 札は `2 版`。読み込んだ範囲に全部が居なければ `版 1/2` と区別して言う
- **選ぶあいだは束ねない。** 束ねたまま 1 枚押すと版ぜんぶが対象になり、
  押した人の思っている範囲と合わない。切り替えも無効にする
- 束ねたカードから開いたときの送り先はその版だけ。一覧ぜんぶだと、まとまって
  見えているのに隣が無関係なものになる
- `版をまとめる` を押せば元の 1 行ずつに戻る

## 2026-09-21 ライブラリの GLB サムネイル（0.28.99）

「MediaForge のライブラリで、GBL ファイルは、軽量なサムネイルで表示できないか」への答え。
**仕組みは既にあり、使われていなかった。**

GLB は絵ではないので、サムネイルは viewer が WebGL で描いた画面を撮って作る
（`assets.model.thumbnail` → `thumbnails.store_model_capture`、上限付きの WebP 1 枚）。
実機を数えると、その撮影は 240 件中 12 件でしか起きていなかった。

```
GLB 240 件 / サムネイル有り 12 件
```
...
`libraryCard` は `preview_kind === "model_3d"` を見ると、**無条件に**画像を隠して「3D」の札を
出していた。server は `library.list` に撮影済みの 160px 版を同梱しているのに、
画面がそれを使わずに捨てていた。12 件は撮れていたのに、一覧では札のままだった。

### 2. 撮影が「viewer で開いたとき」しか起きなかった

240 件を絵にするには 240 回カードを開く必要があった。

### 押されたときにだけ撮る

一覧を開くだけで撮りに行く形にはしなかった。既存の E2E が
`3D runtime was fetched before a model was opened` を検査していて、これは
「一覧は WebGL の束も GLB のバイト列も運ばない」という設計判断である。
iPhone から使う話が出ているので、勝手に運ぶ側へ倒すのは筋が悪い。

代わりに、絵の無い GLB が残っているときだけ一覧に行が出る。

```
絵の無い 3D が 37 件あります。          [3D の絵を作る]
```

押すと 1 件ずつ、既存の撮影経路をそのまま裏で通す。新しい信頼は増えない
（送るのは今までと同じ、上限付きの WebP 1 枚である）。増えるのは GLB を運ぶ費用なので、
そこに歯止めを置いた。

- 16 MiB を超える GLB は撮らない（36 MB を base64 で運ぶ価値はない）
- 1 回に 24 件まで。「もっと見る」で増えたぶんは走っている間に拾う
- 撮れなかったものは覚えて、次の周で選び直さない
- 一覧が入れ替わったら世代が進み、途中で降りる。降りただけのものは覚えない
- 撮れた 1 枚だけ差し替える。一覧を読み直すと撮るたびに全カードが作り直される

撮影ごとに canvas を作り直している。`dispose()` は `renderer.forceContextLoss()` まで
行うので、同じ canvas を使い回すと 2 枚目から context が取れない。`display:none` は
`getBoundingClientRect` が 0 になり viewer が 1px で描くので、画面の外に置いて
大きさだけ本物にしている。

モデルのバイト列を集める処理は `streamModelBytes` として 1 か所に出した。viewer と
一覧の裏取りが別々に組み立てると、片方だけ検査（offset / total_bytes / 空 chunk の
突き合わせ）が抜ける。契約テストで `assets.model.bytes` の呼び出しが 1 か所だけである
ことを固定した。

## 2026-09-21 G9 生成モデルに骨を入れて歩かせる（0.28.95〜0.28.98）

利用者の「骨組み入れて歩くアニメーションできる？」から始まり、生成した 3D に骨を入れて
歩行ループを付けるところまで通した。途中で 3 つの壁があり、それぞれ製品側を直した。

### 壁1: 骨入れの上限を越えられない → `mesh.decimate`（0.28.95）

生成物は密な 1 枚メッシュで届く（512 で 127,796 頂点、1024 で 746,488 頂点）。
`skin.bind_auto` は 5 万頂点 / 10 万ポリゴンが上限で、recipe には面数を落とす操作が無かった。
trellis の `--decim`（格子まとめ）で落とすと胴が虫食いになる。`xatlas` でも `box-uv` でも
同じだったので UV の作り方は原因ではなく、格子削減そのものが薄い装甲板を飲んでいた。

| 方式 | 面 | 頂点 | 見た目 |
|---|---:|---:|---|
| trellis `--decim 48` + box-uv | 37,636 | 37,264 | 虫食い |
| trellis `--decim 48` + xatlas | 37,636 | 87,859 | 虫食い（同じ） |
| Blender collapse 0.22 | 28,931 | 52,098 | パネル・脚のライン保持 |

面数がむしろ少ない collapse が形を保つ。`mesh.decimate` は修飾子をその場で適用し、
下限を割る削減・生成修飾子/shape key 持ち・無変化を拒否する。

### 壁2: bone heat が 1 頂点も解けない → `mesh.weld`（0.28.96）

骨の位置の問題だと思い胴と触角の骨を足したが変わらず、Blender で直接測った。

```
DIAG verts=47262 faces=23671 loose_verts=16902 non_manifold_edges=40059 shells=11466
Bone Heat Weighting: failed to find solution for one or more bones
BIND unweighted_vertices 47262 of 47262
```

**メッシュが 11,466 個の破片に分かれていた。** glTF は UV の継ぎ目で頂点を複製し、
Blender の取り込みは繋ぎ直さない。面が繋がっていないので bone heat は解を持たない。
位置で繋ぐと破片 1 つ・浮き頂点 0・重み無し 0 になった。UV は面の角に持つので見た目は変わらない。

**繋ぐだけで頂点が 127,796 → 61,470 と 52% 減る。** 後の削減は軽くてよい。

| 手順 | 面 | 頂点 | 見た目 |
|---|---:|---:|---|
| weld のみ | 130,656 | 61,470 | 変化なし |
| weld + 0.6 削減 | 78,392 | 35,338 | ほぼ見分けがつかない |
| 繋がずに 0.18 削減 | 23,671 | 47,262 | 骨が入らない |
| 繋いでから 0.18 削減 | 25,826 | 9,041 | 形が壊れる |

### 壁3: 重み検査が正規化前に 1 を強制 → 上限 1000 へ（0.28.97）

Blender の bone heat は 1 を超える重みを返す。実測で 35,338 頂点のうち 1,283 頂点が範囲外、
最大 2.12。`normalized_influences` は直後に合計で割るので絶対値は結果に影響しないのに、
その手前で `0 <= weight <= 1` を強制していて骨入れ全体が `auto_weights_invalid` で落ちていた。
上限を 1000 にした。効くのは有限・非負・少なくとも 1 つが正であること。同じ入力で `failing {}`。

### 実機の結果

正規 Host 経路の 1 回の scene edit（weld → decimate 0.6 → armature → bind_auto → animation.clip）で
scene_384d6ddc1b834c75b18e66dab9022817「R05 WEAVER 歩行リグ」の版 2 が succeeded。
...
| 三角形 | 131,508 | 78,392 |
| オブジェクト | 1 | 2 |
| 骨 | 0 | 17（root / 触角4 / 脚6×2） |
| クリップ | なし | walk 24フレーム @24fps ループ |

骨の位置は当て推量ではなく実形状から測った。上方向は glTF の +Y（下部にちょうど 6 つの塊、
上部は胴 1 つ、最上部に触角 4 つ）。脚の方位は -123.8 / -56.1 / 2.0 / 44.7 / 135.1 / 177.9 度。
Blender は +Z 上なので `(x, y, z)_gltf -> (x, -z, y)_blender` を当てて渡した。

書き出した GLB は `skins: 1 / joints: 17`、`JOINTS_0`・`WEIGHTS_0` 属性、
`mf.walk`（51 チャンネル / 0〜24 フレーム）を持つ。Blender で読み直してフレーム 0/6/12/18 を
描画し、姿勢が実際に変わることを確認した（三脚歩行で常に 3 本接地）。

### 壁4: 画面から recipe を実行できない → `scenes.simplify`（0.28.98）

`mesh.weld` / `mesh.decimate` は `media.scene.edit` で MCP から使えたが、**workspace には
recipe を実行する経路が一つも無かった**ので画面から軽量化できなかった。
`scenes.simplify`（`scene_id` / `base_revision_id` / `ratio`）を足した。画面が送るのは強さだけで、
recipe の組み立てはサーバ側。画面に任意の recipe を撃つ力は与えていない。通常の scene edit
ジョブとして投げるので `scenes.jobs.get` で追える。成功すると新しい版 = ライブラリ登録になる。
画面は viewer が実際に読んだ三角形数に対する見込みだけを出し、読んでいなければ割合だけ言う。

### テクスチャの軽量高品質化は未着手（測って分かったこと）

「ローポリ化すると画像が荒くなる」への調査。**テクスチャは GLB の 5% しかない**
（5,986,092 B のうち 316,566 B、1024² WebP が 2 枚）。残り 95% は幾何なので、軽量化は
weld + decimate が本体で、テクスチャ圧縮の効きは小さい。

見た目の劣化に効くのは高精細→低ポリの法線マップ焼き込みで、`media.scene.bake` として
既に存在する（`high_source` 付き、CPU、256/512/1024）。ただし対象にできるのは
`scene_curves.MAX_VERTICES = 16,384` 頂点 / 32,768 面までで、生成モデル（35,338 頂点 /
78,392 面）は `mesh_geometry` facts に載らず `geometry_sha256` を取れない。
実機で `mesh_geometry: []` を確認した。生成モデルへ焼くには、この facts の上限を
上げる必要がある。上限は手描きの procedural メッシュ向けの設計なので、別スライスとする。

NOT TESTED: 実 iPhone、実 Blender セッションへの指入力、歩行の足滑り/接地の質、
エンジンでの再生、法線焼き込みの生成モデルへの適用。
...
詳細: [HR softmax診断と実測](implementation/g9-pixal-vulkan-softmax-20260920.md)。全体目標は未完了。
...
詳細: [署名release・実生成・Host画面・再起動受入](implementation/g9-installed-acceptance-20260920.md)。
...
詳細: [workflow priority修正](implementation/g9-workflow-priority-20260920.md)。
...
詳細: [release実機記録](implementation/release-0.28.88-acceptance-20260920.md)。証跡はmanaged maintenance/release-0.28.88-20260920。
...
詳細は [trained評価記録](implementation/g9-trained-models-20260919.md)。モデルlockへ11役割の配布identityを固定。
...
[詳細・次の優先順](implementation/installed-reference-acceptance-20260918.md#completed-short-request-pilot-and-quality-comparison)。
...
詳細は[実装・実機記録](implementation/m6-motion-20260918.md)。一時source serverは停止済み。
...
詳細は[実装・実機記録](implementation/m5-deformation-20260918.md)、証跡deformation-source。
...
記録したが、これは誤りだった。降ろしていたのは遊休解放であって、まとめ生成ではない。

ControlDeckのログが答えだった:

  00:05:39 INFO control_deck.llama: idle llama.cpp instance unloaded: Qwen3.8-27B

受入試験がsystemctlで直に起こしたinstanceは、ControlDeck経由の利用としてlast_used_atが
更新されない。巡回（60秒ごと）は毎回「30分使われていない」と判定し、
_has_connected_clientsも偽なので降ろす。落ちる時刻が2秒・19秒・32秒・49秒とばらついたのは
巡回の位相である。画像生成とは関係が無かった。

接続を張ったまま測り直すと、4枚とも成功して言語モデルは載ったままである（45.6秒、山
29.4GiB）。受入6項目も通る（1枚目15秒、2枚目16秒、まとめ4枚30秒）。試験の側を直した——
張りっぱなしの接続で会話を模す。怠ると、検査4は「画像生成が言語モデルを押し出さないこと」
ではなく「巡回に当たらなかったこと」を測る。

測ったVRAMは正しく、こちらは残る。同じpipelineで1〜4枚、枠8.8GiB:

  枚数  allocated  reserved  driver山
    1    6.15GiB   6.73GiB   21.27GiB
    2    6.15      8.50       9.17
    3    6.15      8.50       9.17
    4    6.15      8.50       9.17

真に要る量は枚数によらず6.15GiBで一定。cpu_offloadなので部品は毎回RAMと往復し積み上がら
ない。reservedが8.50で頭打ちなのはworkerへ渡している枠が8.8GiBだから。driverの21.27GiBは
初回生成中だけ現れるtorch外の作業領域で、HIPの起動と小さな行列積・畳み込みだけなら
0.45GiBなので模型の最初の推論で確保される。要件ではない——言語モデルが21.1GiB載って
空きが10.7GiBしかなくても4枚とも通る。

leaseの持ち回り自体は入れた。症状の原因ではなかったが、振る舞いとしては正しい。以前は
件ごとに返して取り直しており、4枚なら4回受付を通していた。返した瞬間brokerから見た空きは
変わらない（workerがVRAMを抱えたまま）のに改めて8.8GiBを求めるので、置けないと判じられる
余地を4回つくっていた。いまは1回である（監査ログで確認: request 1回、activate 1回、
renew 2回、release 1回、その間に4件）。

0.28.78は実機で効かなかった。引き継ぎの判断に_warm_modelを使ったのが誤りで、あれは
「重みをVRAMに抱えているか」であり部分退避では毎回Noneになる。実運用のFluxはcpu_offloadなので
一度も引き継がず、持ち回ったleaseを抱えたまま新しいleaseも取りに行っていた。同じdeviceに
2つぶんを求めることになり、1つぶんより入らない。0.28.79で_carried_modelとして別に持ち、
引き継がないと決めたときは必ず返すようにした。carried/adopted/not adoptedをログに出す。

残る課題: 言語モデルの推論直後の生成が、まれに109秒かかる（通常4〜16秒）。allocatorが
抱える8.50GiBに対し枠が8.8GiBで、利用率97%になっている。空きが動く縁でtorchが確保と解放を
繰り返す、0.28.75で見たのと同じ形である。単発で、続く4枚のbatchは30秒で通った。

## 2026-09-11 int8の重みを二つとも保存する（0.28.76〜0.28.77）

山を決めているのはtransformerであってtext_encoderではない。実測でtransformerは
7.22GiB、int8のtext_encoderは4.47GiB。0.28.76でtransformerもint8にした。

  bf16  山 7.68GiB  生成 6.2〜14.7秒
  int8  山 5.57GiB  生成 4.4〜 4.6秒

「transformerをint8にすると絵が別物になる」と以前書いたのは、プロンプトの違う二枚を
比べた誤りだった。seedと手数と題材を揃えて三題で見比べると差は実用上ない。空いた
2.11GiBは言語モデルの窓に回り、ctxを65,536から131,072へ倍にできた。圧縮までの余裕が
36,590→97,126トークン。

0.28.77でtransformerの量子化結果も保存する。0.28.74でtext_encoderだけを保存しており、
transformerは毎回その場で量子化していた（3.4秒、変換中は元と先が同時にRAMへ乗る）。
置き場をquantized-text-encodersからquantized-weightsへ改め、部品ごとに分ける。

quantoのQuantizedDiffusersModelは抽象の土台で、どの型かを名乗る派生が要る。名乗らないと
save_pretrainedは通るのにfrom_pretrainedだけがValueErrorになり、毎回3.61GiBを書いては
読めずに捨てる。base_class = Flux2Transformer2DModelを名乗って解決。保存2.7秒は初回だけ、
以降は読み直し0.0秒。試験でbase_classの明示を縛った。

VAEは0.157GiB。int8にすると0.078GiBになり絵も変わらない（PSNR 52.7〜53.9dB、差分を16倍に
強調してもほぼ真っ黒）が、得るのは79MiB——山8.9GiBの0.9%でしかないので採らない。

## 2026-09-11 言語モデルと同居して絵を描く（0.28.72〜0.28.75）

base PR503/504/505/506、0.28.71から0.28.75。生成のたびに言語モデルを降ろしていた
のをやめ、同じcardに載せたまま描けるようにした。実測で1枚あたり約70秒が15秒になった。

FLUX.2は重みの8割がtext_encoder（Qwen3）で、動くのは最初の一度だけ。部分退避
（cpu_offload）で山が15.30→7.68GiB、言語モデル18.9GiBと足して収まる。text_encoderは
int8、埋め込みの相対誤差3%で絵の違いは見て取れない。int4は誤差14%で絵が別物になり、
読み込みが6.6倍遅く、得るのは1.8GiBだけなので採らない。transformerのint8は当初
「絵が変わるので採らない」と書いたが、これは0.28.76で覆した（下記）。

0.28.72で入れた「使用後に手放す」がまとめ生成を見ておらず、4枚のbatchで2枚目147秒、
3枚目は8分以上そのままになった。coreにkeep_worker_warm()があり「batchの間は抱える」と
決めていたのにworkerへ伝わっていなかった。0.28.73で伝えるようにし、2枚目以降5秒。

int8の重みを保存して使い回す（0.28.74）。読む＋量子化6.9秒がread0.1秒になり、RAMの山が
18GB→4.5GB。30GBの機械で言語モデルが15GBをmmapしているので、この山が逼迫の根だった。

申告した山7.68GiBは言語モデルのいない状態で測った値で、同居時は8.9GiB使う。枠9.48GiBの
94%を占め、推論でGPU側の空きが動くとtorchが縁で確保と解放を繰り返し、生成が15秒から
63〜93秒になった。0.28.75で山8.9/余白3.0GiBに直し、推論直後でも15〜22秒。

同じ形の欠陥を五つ直した——契約の説明がdescriptionとschemaの二か所、runtime_optionsの
許可がcoreとworkerの二か所、実測が載せ方に追随しない、カタログの指定がdescriptorを
経由しない、batch中の保持がworkerへ届かない。五つとも試験で縛った。測った条件と使う
条件が違ったものが二つ（transformerの大きさ、山の申告）。

受け入れ検査はまとめ生成を含む6項目。1枚ずつしか見ておらず、まとめ生成が固まっても
「導入してよい」と出ていた。

## 残課題: batchの符号化をまとめる

いまbatchはN件をそれぞれ独立したjobとしてcore→workerに通す。workerは次に何が来るかを
知らないので、1件ごとにencoderをGPUへ載せ、符号化し、降ろし、transformerを載せ、描き、
降ろす。N件ぶんの符号化を一度に済ませれば載せ替えが2回に畳める。

  いま        30枚で 27 + 29×5 = 172秒
  まとめると   10 + 10 + 30×2.7 ≈ 100秒（-42%）

代償はbatch全体をworkerの一回の呼び出しにすること。submit_hosted/wait_for_terminal/
wait_cleanupを通る16箇所と、進捗報告・件ごとの失敗の切り分け・資源の貸し借りを作り直す。
今日の欠陥はこの層で起きている。30枚を一度に作る場面が実際に来てから着手する。
